...
int main(){
    ...
    sprintf(p,"%s/timing.txt",base);f=fopen(p,"w");if(f){fprintf(f,"group,algo2_ns,cardano_ns\n");fclose(f);}

    typedef struct{const char*g;const char*e[8];int n;}TG;
    TG ts[]={{"diag",{"1e-4"},1},{"ps1",{"1e-4"},1},{"ps2",{"1e-4"},1},{"ps3",{"1e-4"},1},{"general",{"1e-4"},1},{"dual",{"1e-4","1e-6"},2},{"sweep/2.5e-3",{"1e-2"},1},{"sweep/2.5e-4",{"1e-3"},1},{"sweep/2.5e-5",{"1e-4"},1},{"sweep/2.5e-6",{"1e-5"},1},{"sweep/2.5e-7",{"1e-6"},1},{NULL,{NULL},0}};
    ...
    for(int gi=0;ts[gi].g;gi++){
        ...
        sprintf(fp2,"%s/timing.txt",base);f=fopen(fp2,"a");if(f){fprintf(f,"%s,%.1f,%.1f\n",ts[gi].g,ta_ns,tc_ns);fclose(f);}
        free(ms);
    }
    printf("\nDone\n");return 0;
}
