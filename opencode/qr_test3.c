...
int main() {
    ...
    // Test 1: Large span [1, 1e12]
    double sum_qr = 0, max_qr = 0;
    for (int i = 0; i < n; i++) {
        double d[3];
        for (int k = 0; k < 3; k++)
            d[k] = pow(10, myrand() * 12);
        double m0 = fmin(fmin(d[0], d[1]), d[2]);
        double a12 = (myrand() * 2 - 1) * 0.49 * m0;
        double a13 = (myrand() * 2 - 1) * 0.49 * m0;
        double a23 = (myrand() * 2 - 1) * 0.49 * m0;
        double m[6] = {d[0], d[1], d[2], a12, a13, a23};
        double rr[6], lr[3], lq[3];
        conv(m, rr); robust(rr, lr); qsort(lr, 3, sizeof(double), cd);
        qrf(m, lq); qsort(lq, 3, sizeof(double), cd);
        double e = fmax(fmax(relerr(lq[0],lr[0]), relerr(lq[1],lr[1])), relerr(lq[2],lr[2]));
        sum_qr += e;
        if (e > max_qr) max_qr = e;
    }
    printf("Span [1,1e12]: QR avg_err=%.2e max_err=%.2e\n", sum_qr/n, max_qr);

    // Test 2: Small span [1, 100]
    sum_qr = 0; max_qr = 0;
    for (int i = 0; i < n; i++) {
        double d[3];
        for (int k = 0; k < 3; k++)
    ...
    printf("Span [1,100]:  QR avg_err=%.2e max_err=%.2e\n", sum_qr/n, max_qr);

    // Test 3: Moderate span [1, 1e6]
    sum_qr = 0; max_qr = 0;
    for (int i = 0; i < n; i++) {
        double d[3];
        for (int k = 0; k < 3; k++)
    ...
    printf("Span [1,1e6]:  QR avg_err=%.2e max_err=%.2e\n", sum_qr/n, max_qr);
