                    ...
                    JX_Int             **CF_marker_ptr )
{
   ...
   *---------------------------------------------------*/
   //choose allreduce
      ...
      global_graph_size = 1;
      MPI_Request *req = jx_CTAlloc(MPI_Request, 1);
      MPI_Status  *status = jx_CTAlloc(MPI_Status, 1);
   #endif

   #if (choosen == 2)
      MPI_Comm  Node_Comm  = jx_hpNodeComm(hp_hardware);
      JX_Int tmp_graph_size;
   #endif
   ...
   while (1)
   {
      ...
      #elif (choosen == 1) 
         jx_My_MPI_Allreduce(&graph_size, &global_graph_size, 1, JX_MPI_INT, MPI_SUM, comm);
      #elif (choosen == 2)
         jx_My_MPI_Iallreduce_First(&graph_size, &global_graph_size, 1, JX_MPI_INT, MPI_SUM, comm);
         if(Node_Comm != MPI_COMM_NULL)  MPI_Iallreduce(&global_graph_size, &tmp_graph_size, 1, JX_MPI_INT, MPI_SUM, Node_Comm, req);
      #elif (choosen == 3)
         MPI_Iallreduce(&graph_size, &global_graph_size, 1, JX_MPI_INT, MPI_SUM, comm, req);
      #elif (choosen == 4)
         MPI_Ireduce(&graph_size, &global_graph_size, 1, JX_MPI_INT, MPI_SUM, num_procs - 1, comm, req);
      #endif

      #if JX_PRINT_MODULE_TIME && JX_USE_HPCTOOLKIT == 0
         jx_sprintf(timetag,"%s_l%d", "Allreduce",TimerLevel);
	 ...
	 *-----------------------------------------------------*/
         ...
         if (!CF_marker[i]==0) /* C or F point */
	 {
      ...
      jx_My_MPI_Iallreduce_Sencond(&tmp_graph_size, 1, MPI_INT, MPI_SUM, comm, req);
      global_graph_size = tmp_graph_size;
   #elif (choosen == 3)
      MPI_Wait(req, status);
   #elif (choosen == 4)
      MPI_Wait(req, status);
      jx_my_bcast(&global_graph_size, 1, MPI_INT, num_procs - 1, comm);
   #endif
   } /* end while */
