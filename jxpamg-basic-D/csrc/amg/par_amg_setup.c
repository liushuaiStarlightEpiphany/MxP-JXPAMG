               ...
               jx_hpCSRMatrix *hp_matrix)
{
   ...
   {
      /* 建议在循环内部定义临时指针，并存入数组 */
      JX_Real *measure_gai = NULL;
      JX_Int *marker_gai = NULL;
      JX_Int *side_gai = NULL;
      JX_Real tau_actual = 0.0;
      ...
      if (max_levels > 1)
      {
         ...
         // jx_PAMGMeasureGAI_Base(par_S,
         //                        jx_hpCSRMatrixPar(A_array[level]),
         //                        level,
         //                        debug_flag,
      ...
      if (restri_type)
      {
         ...
         if (num_procs > 1)
            jx_MatvecCommPkgCreate(jx_hpCSRMatrixPar(A_H));
         /* Delete AP */
...
/*Create hierarchy block of coarse matrix*/
   ...
   if (amg_print_level > 0 && ai_measure_type)
   {
      ...
      if (my_id == 0)
      {
         ...
         if (num_ai)
            jx_printf(" - num_ai_c_ratio = %f\n", num_ai_c / num_ai);
         if (num_ai_valid)
            jx_printf(" - num_ai_c_ratio_valid = %f\n", num_ai_c / num_ai_valid);
         if (mai_global > 0.0)
            jx_printf(" - measure_ai_ratio_valid = %f\n", mai_global_valid / mai_global);
         if (mai_global > 0.0)
            jx_printf(" - measure_ai_c_ratio = %f\n", mai_c_global / mai_global);
         if (mai_global_valid > 0.0)
            jx_printf(" - measure_ai_c_ratio_valid = %f\n", mai_c_global / mai_global_valid);
      }
