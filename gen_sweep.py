...
for ri, (ratio, name) in enumerate(zip(ratios, names)):
    for i in range(100):
        a11 = 10**random.uniform(0, 12)
        a22 = 10**random.uniform(0, 12)
        a33 = 10**random.uniform(0, 12)
        bound12 = ratio * min(a11, a22)
        bound13 = ratio * min(a11, a33)
        bound23 = ratio * min(a22, a33)
        r12 = random.uniform(-1, 1) * bound12
        r13 = random.uniform(-1, 1) * bound13
        r23 = random.uniform(-1, 1) * bound23
...
print('Total: %d files' % (sum(len(os.listdir(os.path.join(base, n))) for n in names)))
