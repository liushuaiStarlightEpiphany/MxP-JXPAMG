...
static void qrf(const double m[6], double l[3])
{
    ...
    for (int it = 0; it < 50; it++) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            for (int j = i; j < 3; j++) {
                double x = A[i*3+j], y = A[(i+1)*3+j];
                A[i*3+j] = c*x - s*y; A[(i+1)*3+j] = s*x + c*y;
            }
            for (int j = 0; j < 3; j++) {
                double x = A[j*3+i], y = A[j*3+i+1];
                A[j*3+i] = c*x - s*y; A[j*3+i+1] = s*x + c*y;
            }
        }
        for (int i = 0; i < 3; i++) A[i*3+i] += sh;
    }
...
int main()
{
    ...
    for (int gi = 0; ts[gi].g; gi++) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            ca = clock(); for (int r = 0; r < rep; r++) a2(m, l, 0); cb = clock();
            tc += (double)(cb - ca);

            ca = clock(); for (int r = 0; r < rep/4; r++) jacobi(m, l); cb = clock();
            tj += (double)(cb - ca);

            ca = clock(); for (int r = 0; r < rep/4; r++) qrf(m, l); cb = clock();
            tq += (double)(cb - ca);
        }
