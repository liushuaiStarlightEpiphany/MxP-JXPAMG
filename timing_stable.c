...
int main(){
    ...
    char p[1024];
    sprintf(p,"%s/timing_stable.txt",base);
    FILE*f=fopen(p,"w");if(f){fprintf(f,"group,algo2_ns,cardano_ns,jacobi_ns,qr_ns\n");fclose(f);}

    typedef struct{const char*g;const char*e;int n;}TG;
    TG ts[]={{"diag","1e-4",500},{"ps1","1e-4",500},{"ps2","1e-4",500},{"ps3","1e-4",500},
             {"general","1e-4",500},{"dual","1e-4",300},
             {"sweep/2.5e-3","1e-2",100},{"sweep/2.5e-4","1e-3",100},{"sweep/2.5e-5","1e-4",100},
             {"sweep/2.5e-6","1e-5",100},{"sweep/2.5e-7","1e-6",100},{NULL,NULL,0}};
    ...
    for(int gi=0;ts[gi].g;gi++){
        ...
        printf("%s (n=%d): ",ts[gi].g,n);
        fflush(stdout);

        /* Run 3 passes and average */
        double sum_a2=0,sum_ca=0,sum_ja=0,sum_qr=0;
        int passes=3;
        for(int ps=0;ps<passes;ps++){
            ...
            sum_a2+=ta/n/rep/CLOCKS_PER_SEC*1e9;
            sum_ca+=tc/n/rep/CLOCKS_PER_SEC*1e9;
            sum_ja+=tj/n/(rep/4)/CLOCKS_PER_SEC*1e9;
            sum_qr+=tq/n/(rep/4)/CLOCKS_PER_SEC*1e9;
        }
        double t_a2=sum_a2/passes,t_ca=sum_ca/passes,t_ja=sum_ja/passes,t_qr=sum_qr/passes;
        printf("a2=%.1fns ca=%.1fns ja=%.1fns qr=%.1fns\n",t_a2,t_ca,t_ja,t_qr);

        sprintf(fp2,"%s/timing_stable.txt",base);
        f=fopen(fp2,"a");if(f){fprintf(f,"%s,%.1f,%.1f,%.1f,%.1f\n",ts[gi].g,t_a2,t_ca,t_ja,t_qr);fclose(f);}
        free(ms);
    }
    printf("\nDone\n");return 0;
}
