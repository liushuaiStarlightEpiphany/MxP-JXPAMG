...
/* convert algo2 format (a11,a22,a33,a12,a13,a23) to robust format (a11,a12,a13,a22,a23,a33) */
static void cv(const double*m,double*r){r[0]=m[0];r[1]=m[3];r[2]=m[4];r[3]=m[1];r[4]=m[5];r[5]=m[2];}

/* robust helpers - copied from benchmark_all.c */
static double tr_r(const double*A){return A[0]+A[3]+A[5];}
static void sh_r(const double*A,double*B){for(int i=0;i<6;i++)B[i]=A[i];double a=tr_r(A)/3;B[0]-=a;B[3]-=a;B[5]-=a;}
static double de_r(const double*A){return A[0]*A[3]*A[5]+A[1]*A[4]*A[2]+A[2]*A[1]*A[4]-A[2]*A[3]*A[2]-A[4]*A[4]*A[0]-A[1]*A[1]*A[5];}
static void mv_r(const double*A,const double*v,double*r){r[0]=A[0]*v[0]+A[1]*v[1]+A[2]*v[2];r[1]=A[1]*v[0]+A[3]*v[1]+A[4]*v[2];r[2]=A[2]*v[0]+A[4]*v[1]+A[5]*v[2];}
static void su_r(const double*A,const double*B,double*C){for(int i=0;i<6;i++)C[i]=A[i]-B[i];}
static void sv_r(const double*a,const double*b,double*c){c[0]=a[0]-b[0];c[1]=a[1]-b[1];c[2]=a[2]-b[2];}
static double mo_r(const double*v){return sqrt(v[0]*v[0]+v[1]*v[1]+v[2]*v[2]);}
static double do_r(const double*a,const double*b){return a[0]*b[0]+a[1]*b[1]+a[2]*b[2];}
static void sc_r(double n,double*v,double*r){r[0]=v[0]*n;r[1]=v[1]*n;r[2]=v[2]*n;}
static int sg_r(double x){return (x>0)-(x<0);}

/* robust (Scherzinger) from benchmark_all.c */
static void robust(const double A[6],double*lambda){
    double A1l1,A1l2,A1l3;
    ...
    A1l2=(A4[1]+A4[4])/2.0-sg_r(A4[1]-A4[4])*sqrt((A4[1]-A4[4])*(A4[1]-A4[4])+4*A4[2]*A4[3])/2.0;
    A1l3=(A4[1]+A4[4])/2.0+sg_r(A4[1]-A4[4])*sqrt((A4[1]-A4[4])*(A4[1]-A4[4])+4*A4[2]*A4[3])/2.0;
    lambda[0]=A1l1+t3;lambda[1]=A1l2+t3;lambda[2]=A1l3+t3;
}

int main(){
    char home[256];sprintf(home,"%s",getenv("HOME"));char base[1024];
    sprintf(base,"%s/l_s/eigenvalue-3/result",home);char p[1024];
    sprintf(p,"%s/exp_data.csv",base);FILE*f=fopen(p,"w");if(f){fprintf(f,"group,eps,case,relerr\n");fclose(f);}
    sprintf(p,"%s/case_distribution.txt",base);f=fopen(p,"w");if(f){fprintf(f,"group,eps,C1,C2,C3,C4,C5,reducible,total,pct\n");fclose(f);}
    sprintf(p,"%s/error_stats.txt",base);f=fopen(p,"w");if(f){fprintf(f,"group,eps,avg_err,max_err\n");fclose(f);}
    sprintf(p,"%s/timing.txt",base);f=fopen(p,"w");if(f){fprintf(f,"group,algo2_ns,cardano_ns,ratio\n");fclose(f);}

    typedef struct{const char*g;const char*e[8];int n;}TG;
    TG ts[]={{"diag",{"1e-4"},1},{"ps1",{"1e-4"},1},{"ps2",{"1e-4"},1},{"ps3",{"1e-4"},1},{"general",{"1e-4"},1},
    ...
    for(int gi=0;ts[gi].g;gi++){
        ...
        for(int ei=0;ei<ts[gi].n;ei++){
            double eps=atof(ts[gi].e[ei]);int cs[6]={0};double se=0,me=0;int v=0;
            double l[3],lr[3],rr[6];
            for(int i=0;i<n;i++){
                double*m=&ms[i*6];int ca=gc(m,eps);cs[ca]++;a2(m,l,eps);cv(m,rr);robust(rr,lr);
                double rel=0;for(int k=0;k<3;k++){double e=fabs(l[k]-lr[k])/(fabs(lr[k])+1e-300);if(e>rel)rel=e;}
                se+=rel;if(rel>me)me=rel;v++;
                sprintf(fp2,"%s/exp_data.csv",base);f=fopen(fp2,"a");if(f){fprintf(f,"%s,%s,%d,%.15e\n",ts[gi].g,ts[gi].e[ei],ca,rel);fclose(f);}
            }
            ...
            sprintf(fp2,"%s/case_distribution.txt",base);f=fopen(fp2,"a");if(f){fprintf(f,"%s,%s,%d,%d,%d,%d,%d,%d,%d,%.0f\n",ts[gi].g,ts[gi].e[ei],cs[1],cs[2],cs[3],cs[4],cs[5],red,v,100.0*red/v);fclose(f);}
            sprintf(fp2,"%s/error_stats.txt",base);f=fopen(fp2,"a");if(f){fprintf(f,"%s,%s,%.2e,%.2e\n",ts[gi].g,ts[gi].e[ei],se/v,me);fclose(f);}
        }
        int rep=20000;clock_t ca,cb;double ta=0,tc=0;double l[3];
        for(int i=0;i<n;i++){double*m=&ms[i*6];ca=clock();for(int r=0;r<rep;r++)a2(m,l,1e-10);cb=clock();ta+=(double)(cb-ca);ca=clock();for(int r=0;r<rep;r++)a2(m,l,0);cb=clock();tc+=(double)(cb-ca);}
        ...
        sprintf(fp2,"%s/timing.txt",base);f=fopen(fp2,"a");if(f){fprintf(f,"%s,%.1f,%.1f,%.2f\n",ts[gi].g,ta_ns,tc_ns,tc_ns/ta_ns);fclose(f);}
        free(ms);
    }
