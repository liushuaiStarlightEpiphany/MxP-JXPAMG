...
#undef CREATE_F_VECTOR
...

   {
    ...
    /* ============================================================
    * HF32/XF32 candidate vs FP64
    * A: FP16 storage, x: FP32 on device
    * ============================================================ */
   if (myid == 0)
      printf("\n================ HF32/XF32 vs FP64 SpMV error ================\n");
   ...
   if (myid == 0)
   {
      printf("\n[HF32/XF32 GSM block4 + dma_sg]\n");
      printf("  mix_spmv_type          : %d\n", mix_spmv_type);
      printf("  jxh_spmv_type          : %d\n", jxh_spmv_type);
      printf("  ||v_xf32_H||_2         : %.12e\n", (double)norm_v2_H);
      printf("  ||FP64-ref - XF32||_2  : %.12e\n", (double)norm_diff_Hmul_vs_D);
...

   {
      ...
      {
         printf("\n----------------------------\n");
         printf("HF32/XF32 method : FP16 storage + FP32 x + FP32 product pair-sum block4 + dma_sg\n");
         printf("mix_spmv_type    : %d\n", mix_spmv_type);
         printf("jxh_spmv_type    : %d\n", jxh_spmv_type);
         printf("HF32/XF32 total  : %e seconds\n", total);
         printf("----------------------------\n");
      }
   }
