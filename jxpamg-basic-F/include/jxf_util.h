...
char *jxf_MAlloc_origin(size_t size);
char *jxf_MAlloc_MT(size_t size);
char *jxf_CAlloc(size_t count, size_t elt_size);
char *jxf_CAlloc_MT(size_t count, size_t elt_size);
char *jxf_CAlloc_origin(size_t count, size_t elt_size);
char *jxf_ReAlloc(char *ptr, size_t size);
char *jxf_ReAlloc_origin(char *ptr, size_t size);
char *jxf_ReAlloc_MT(char *ptr, size_t size);
void *jxf_ReAlloc_v2(void *ptr, size_t old_size, size_t new_size);
void jxf_Free(char *ptr);
void jxf_Free_origin(char *ptr);
void jxf_Free_MT(char *ptr);
...
#define jxf_assert(EX)                                     \
   if (!(EX))                                              \
   {                                                       \
      jxf_fprintf(stderr, "jxf_assert failed: %s\n", #EX); \
      jxf_error(1);                                        \
   }
#endif
...
void jxf_GetWallTime(MPI_Comm comm,
                     char *fctname,
                     JXF_Real starttime,
                     JXF_Real endtime,
                     JXF_Int allproc,
                     JXF_Int dp);
void jxf_GetWallTime2(MPI_Comm comm, char *fctname, JXF_Real totaltime, JXF_Int allproc, JXF_Int dp);
JXF_Real
...
void jxf_GetSimpleThreadPartition(JXF_Int *begin, JXF_Int *end, JXF_Int n);

/* csrc/utilities/prefixsum.c */
void jxf_prefix_sum_pair(JXF_Int *in_out1, JXF_Int *sum1, JXF_Int *in_out2, JXF_Int *sum2, JXF_Int *workspace);
