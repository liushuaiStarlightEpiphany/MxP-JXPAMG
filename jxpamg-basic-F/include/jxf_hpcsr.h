...
typedef struct
{
   jxh_ParCSRMatrix *par_matrix;

   /*
    * 当前 half 版本暂时不使用 CPU / Node / Core 分级矩阵。
    * 为了减少改动，先保留字段，但初始化为 NULL。
    */
   jxf_hpCSRMatrixLevel *node_matrix;
   jxf_hpCSRMatrixLevel *cpu_matrix;
   jxf_hpCSRMatrixLevel *core_matrix;
...
/* hpcsr basic access */
#define jxh_hpCSRMatrixPar(matrix) ((matrix)->par_matrix)
#define jxh_hpCSRMatrixCpu(matrix) ((matrix)->cpu_matrix)
#define jxh_hpCSRMatrixNode(matrix) ((matrix)->node_matrix)
#define jxh_hpCSRMatrixCore(matrix) ((matrix)->core_matrix)
#define jxh_hpCSRMatrixComm(matrix) ((matrix)->par_matrix->comm)
#define jxh_hpCSRMatrixGlobalNumRows(matrix) ((matrix)->par_matrix->global_num_rows)
#define jxh_hpCSRMatrixGlobalNumCols(matrix) ((matrix)->par_matrix->global_num_cols)
#define jxh_hpCSRMatrixFirstRowIndex(matrix) ((matrix)->par_matrix->first_row_index)
#define jxh_hpCSRMatrixFirstColDiag(matrix) ((matrix)->par_matrix->first_col_diag)
#define jxh_hpCSRMatrixLastRowIndex(matrix) ((matrix)->par_matrix->last_row_index)
#define jxh_hpCSRMatrixLastColDiag(matrix) ((matrix)->par_matrix->last_col_diag)
/* diag/offd */
#define jxh_hpCSRMatrixDiag(matrix) ((matrix)->par_matrix->diag)
#define jxh_hpCSRMatrixOffd(matrix) ((matrix)->par_matrix->offd)
#define jxh_hpCSRMatrixColMapOffd(matrix) ((matrix)->par_matrix->col_map_offd)
/* partition */
#define jxh_hpCSRMatrixRowStarts(matrix) ((matrix)->par_matrix->row_starts)
#define jxh_hpCSRMatrixColStarts(matrix) ((matrix)->par_matrix->col_starts)
/* communication */
#define jxh_hpCSRMatrixCommPkg(matrix) ((matrix)->par_matrix->comm_pkg)
/* ownership */
#define jxh_hpCSRMatrixOwnsData(matrix) ((matrix)->par_matrix->owns_data)
#define jxh_hpCSRMatrixOwnsRowStarts(matrix) ((matrix)->par_matrix->owns_row_starts)
#define jxh_hpCSRMatrixOwnsColStarts(matrix) ((matrix)->par_matrix->owns_col_starts)
/* size / nnz */
#define jxh_hpCSRMatrixNumRows(matrix) (jxh_CSRMatrixNumRows((matrix)->par_matrix->diag))
#define jxh_hpCSRMatrixNumCols(matrix) (jxh_CSRMatrixNumCols((matrix)->par_matrix->diag))
#define jxh_hpCSRMatrixNumNonzeros(matrix) ((matrix)->par_matrix->num_nonzeros)
#define jxh_hpCSRMatrixDNumNonzeros(matrix) ((matrix)->par_matrix->d_num_nonzeros)
/* getrow related */
#define jxh_hpCSRMatrixRowindices(matrix) ((matrix)->par_matrix->rowindices)
#define jxh_hpCSRMatrixRowvalues(matrix) ((matrix)->par_matrix->rowvalues)
#define jxh_hpCSRMatrixGetrowactive(matrix) ((matrix)->par_matrix->getrowactive)
#define jxh_hpCSRMatrixAssumedPartition(matrix) ((matrix)->par_matrix->assumed_partition)
...
JXF_Int jxh_hpCSRMatrixDestroy(jxh_hpCSRMatrix *matrix);
...
JXF_Int jxh_hpCSRMatrixMatvec(JXF_Real alpha, jxh_hpCSRMatrix *A, jxf_ParVector *x, JXF_Real beta, jxf_ParVector *y);
...
jxf_hpPAMGRelax3(jxf_hpCSRMatrix *hp_matrix,
                 ...
                 jxf_ParVector *Vtemp);
JXF_Int
jxh_hpPAMGRelax3(jxh_hpCSRMatrix *hp_matrix,
                 jxf_ParVector *par_rhs,
                 JXF_Int *cf_marker,
                 JXF_Int relax_points,
                 JXF_Real relax_weight,
                 ...
                 jxf_ParVector *Vtemp);
JXF_Int
jxh_hpPAMGRelax3_HmulFacc(jxh_hpCSRMatrix *hp_matrix,
                          jxf_ParVector *par_rhs,
                          JXF_Int *cf_marker,
                          JXF_Int relax_points,
                          JXF_Real relax_weight,
...
jxf_hpPAMGRelax9(jxf_hpCSRMatrix *hp_matrix,
                 ...
                 jxf_ParVector *Vtemp);
JXF_Int
jxh_hpPAMGRelax9(jxh_hpCSRMatrix *hp_matrix,
                 jxf_ParVector *par_rhs,
                 JXF_Int *cf_marker,
                 JXF_Int relax_points,
                 JXF_Real relax_weight,
