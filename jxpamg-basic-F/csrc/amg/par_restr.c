                           ...
                           JXF_Int            *R_max_size_ptr )
{
   ...
   jxf_hpIfReorderMatvecCommPkg(comm_pkg_SF)   = 0;
   jxf_hpCSRCommPkgReorder(comm_pkg_SF);
   #endif
   ...
   #if JXF_REODER_SEND_RECV
   jxf_hpIfReorderMatvecCommPkg(comm_pkg_FF2_j)   = 0;
   jxf_hpCSRCommPkgReorder(comm_pkg_FF2_j);
   #endif  
...
jxf_PAMGBuildOPntRestr( jxf_ParCSRMatrix *A, JXF_Int *CF_marker, JXF_Int *num_cpts_global, jxf_ParCSRMatrix **R_ptr, JXF_Int *R_max_size_ptr )
{
   ...
   if (my_id == 0)
   {
      R_diag = jxf_CSRMatrixCreate(total_global_cpts, n_fine, total_global_cpts);
      jxf_CSRMatrixInitialize(R_diag);
      for (i = 0; i <= total_global_cpts; i ++) jxf_CSRMatrixI(R_diag)[i] = i;
      j = 0;
      for (i = 0; i < n_fine; i ++)
      {
         if (CF_marker[i] == 1)
         {
            jxf_CSRMatrixJ(R_diag)[j] = i;
            jxf_CSRMatrixData(R_diag)[j] = 1.0;
            j ++;
         }
      }
   }

  *R_ptr = jxf_CSRMatrixToParCSRMatrix(comm, R_diag, NULL, NULL);
  *R_max_size_ptr = 1;
   if (my_id == 0)
   {
