...
#define SQR(a) ((a)*(a))
static double pythag(double a,double b){double aa=fabs(a),bb=fabs(b);if(aa>bb)return aa*sqrt(1.0+SQR(bb/aa));else return(bb==0.0?0.0:bb*sqrt(1.0+SQR(aa/bb)));}
...
static void a2(const double m[6],double l[3],double eps){
    ...
    double rr[6];rr[0]=m[0];rr[1]=m[3];rr[2]=m[4];rr[3]=m[1];rr[4]=m[5];rr[5]=m[2];
    robust(rr,l);qsort(l,3,sizeof(double),cd);
}
...
static void sym_ql(const double m[6],double l[3]){
    double a[9]={m[0],m[3],m[4],m[3],m[1],m[5],m[4],m[5],m[2]},d[3],e[3];
    int n=3;
    double scale,hh,h,g,f;
    for(int i=n-1;i>0;i--){int l=i-1;h=scale=0.0;
        if(l>0){for(int k=0;k<=l;k++)scale+=fabs(a[i*3+k]);
            if(scale==0.0)e[i]=a[i*3+l];else{for(int k=0;k<=l;k++){a[i*3+k]/=scale;h+=a[i*3+k]*a[i*3+k];}
                f=a[i*3+l];g=(f>=0.0?-sqrt(h):sqrt(h));e[i]=scale*g;h-=f*g;a[i*3+l]=f-g;f=0.0;
                for(int j=0;j<=l;j++){a[j*3+i]=a[i*3+j]/h;g=0.0;
                    for(int k=0;k<=j;k++)g+=a[j*3+k]*a[i*3+k];for(int k=j+1;k<=l;k++)g+=a[k*3+j]*a[i*3+k];
                    e[j]=g/h;f+=e[j]*a[i*3+j];}
                hh=f/(h+h);for(int j=0;j<=l;j++){f=a[i*3+j];e[j]=g=e[j]-hh*f;
                    for(int k=0;k<=j;k++)a[j*3+k]-=(f*e[k]+g*a[i*3+k]);}}
        }else e[i]=a[i*3+l];d[i]=h;
    }d[0]=0.0;e[0]=0.0;
    for(int i=0;i<n;i++){int l=i;if(d[i]!=0.0)for(int j=0;j<l;j++){g=0.0;for(int k=0;k<l;k++)g+=a[i*3+k]*a[k*3+j];
        for(int k=0;k<l;k++)a[k*3+j]-=g*a[k*3+i];}
        d[i]=a[i*3+i];a[i*3+i]=1.0;for(int j=0;j<l;j++)a[j*3+i]=a[i*3+j]=0.0;}
    double s,r,p,dd,c,b, *ee=e;
    for(int i=1;i<n;i++)ee[i-1]=ee[i];ee[n-1]=0.0;
    for(int l=0;l<n;l++){int iter=0,m,i;
        do{for(m=l;m<n-1;m++){dd=fabs(d[m])+fabs(d[m+1]);if(fabs(ee[m])+dd==dd)break;}
            if(m!=l){if(iter++==100)break;
                g=(d[l+1]-d[l])/(2.0*ee[l]);r=pythag(g,1.0);
                g=d[m]-d[l]+ee[l]/(g+(g>=0.0?fabs(r):-fabs(r)));
                s=c=1.0;p=0.0;
                for(i=m-1;i>=l;i--){f=s*ee[i];b=c*ee[i];ee[i+1]=(r=pythag(f,g));
                    if(r==0.0){d[i+1]-=p;ee[m]=0.0;break;}
                    s=f/r;c=g/r;g=d[i+1]-p;r=(d[i]-g)*s+2.0*c*b;d[i+1]=g+(p=s*r);g=c*r-b;}
                if(r==0.0&&i>=l)continue;d[l]-=p;ee[l]=g;ee[m]=0.0;}
        }while(m!=l);}
    l[0]=d[0];l[1]=d[1];l[2]=d[2];qsort(l,3,sizeof(double),cd);
}
...
int main(){
    ...
    for(int gi=0;ts[gi].g;gi++){
        ...
        for(int i=0;i<n;i++){
            ...
            ca=clock();for(int r=0;r<rep;r++)cardano_a2(m,l);cb=clock();tc+=(double)(cb-ca);
            ca=clock();for(int r=0;r<rep/4;r++)jacobi(m,l);cb=clock();tj+=(double)(cb-ca);
            ca=clock();for(int r=0;r<rep/4;r++)sym_ql(m,l);cb=clock();tq+=(double)(cb-ca);
        }
