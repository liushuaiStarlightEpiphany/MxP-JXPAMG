              ...
              jx_ParVector    *Ztemp )
{
   ...
   *-----------------------------------------------------------------------------*/
   ...
   if (relax_type == 0)
   {
      relax_error = jx_hpPAMGRelax0(hp_matrix, par_rhs, cf_marker, relax_points,
                                  relax_weight, omega, par_app, Vtemp);
   }
   else if (relax_type == 1)
   {
      relax_error = jx_hpPAMGRelax1(hp_matrix, par_rhs, cf_marker, relax_points,
                                  relax_weight, omega, par_app, Vtemp);
   }
   else if (relax_type == 2)
   {
      relax_error = jx_hpPAMGRelax2(hp_matrix, par_rhs, cf_marker, relax_points,
                                  relax_weight, omega, par_app, Vtemp);
   }
   else if (relax_type == 3)
   {
      relax_error = jx_hpPAMGRelax3(hp_matrix, par_rhs, cf_marker, relax_points,
                                  relax_weight, omega, par_app, Vtemp);
   }
   else if (relax_type == 4)
   {
      relax_error = jx_hpPAMGRelax4(hp_matrix, par_rhs, cf_marker, relax_points,
                                  relax_weight, omega, par_app, Vtemp);
   }
   else if (relax_type == 5)
   {
      relax_error = jx_hpPAMGRelax5(hp_matrix, par_rhs, cf_marker, relax_points,
                                  relax_weight, omega, par_app, Vtemp);
   }
   else if (relax_type == 6)
   {
      relax_error = jx_hpPAMGRelax6(hp_matrix, par_rhs, cf_marker, relax_points,
                                  relax_weight, omega, par_app, Vtemp);
   }
   else if (relax_type == 7)
   {
      relax_error = jx_hpPAMGRelax7(hp_matrix, par_rhs, cf_marker, relax_points,
                                  relax_weight, omega, par_app, Vtemp);
   }
   else if (relax_type == 8)
   {
      relax_error = jx_hpPAMGRelax8(hp_matrix, par_rhs, cf_marker, relax_points,
                                  relax_weight, omega, l1_norms, par_app, Vtemp, Ztemp);   
   }
   else if (relax_type == 9)
   {
      relax_error = jx_hpPAMGRelax9(hp_matrix, par_rhs, cf_marker, relax_points,
                                  relax_weight, omega, par_app, Vtemp);
   }

   else if (relax_type == 13)
   {
      relax_error = jx_hpPAMGRelax13(hp_matrix, par_rhs, cf_marker, relax_points,
                                   relax_weight, omega, l1_norms, par_app, Vtemp, Ztemp);
   }
   else if (relax_type == 14)
   {
      relax_error = jx_hpPAMGRelax14(hp_matrix, par_rhs, cf_marker, relax_points,
                                   relax_weight, omega, l1_norms, par_app, Vtemp, Ztemp);
   }
