...
void a2(const double m[6],double l[3],double eps){
    ...
    double q2=m[0]*m[1]*m[2]+2*m[3]*m[5]*m[4]-m[0]*m[5]*m[5]-m[1]*m[4]*m[4]-m[2]*m[3]*m[3]+2*t3*t3*t3-t3*(m[0]*m[1]+m[0]*m[2]+m[1]*m[2]-m[3]*m[3]-m[4]*m[4]-m[5]*m[5]);
    double dp=p/3,dq=-q2/2,dis=dq*dq+dp*dp*dp;
...
extern void robust(const double A[6],double lambda[3]);

int main(){
    double m[6]={1.0e10,1.0e0,1.0e5,1.0e-2,1.0e-2,1.0e-2};
    double l[3],lr[3],rr[6];
    a2(m,l,1e-4);
    rr[0]=m[0];rr[1]=m[3];rr[2]=m[4];rr[3]=m[1];rr[4]=m[5];rr[5]=m[2];
    robust(rr,lr);
    printf("algo2: %.15e %.15e %.15e\n",l[0],l[1],l[2]);
    printf("robust: %.15e %.15e %.15e\n",lr[0],lr[1],lr[2]);
