...
void scan_dir(const char *path,double*mats,int*n,int max_n){
    ...
    while((entry=readdir(dp))&&*n<max_n){
        ...
        sprintf(fpath,"%s/%s",path,entry->d_name);double m[6];
        if(read_mtx(fpath,m)){for(int i=0;i<6;i++)mats[(*n)*6+i]=m[i];(*n)++;}
    }
    closedir(dp);
}

typedef struct{const char*group;const char*epslist[8];int neps;} TestGroup;

int main(){
    char home[256];sprintf(home,"%s",getenv("HOME"));
    
    // Define per-group test epsilon values
    TestGroup tests[]={
        {"diag",             {"1e-4"}, 1},
        {"ps1",              {"1e-4"}, 1},
        {"ps2",              {"1e-4"}, 1},
    ...
    FILE*fp=fopen(outpath,"w");if(fp){fprintf(fp,"group,eps,case,relerr\n");fclose(fp);}
    ...
    for(int gi=0;tests[gi].group;gi++){
        char path[1024];
        sprintf(path,"%s/l_s/eigenvalue-3/matrix-test/%s",home,tests[gi].group);
        int n=0;double*mats=(double*)malloc(10000*6*sizeof(double));
        scan_dir(path,mats,&n,10000);
        if(!n){free(mats);continue;}
        
        printf("\n========== %s (n=%d) ==========\n",tests[gi].group,n);
        
        for(int ei=0;ei<tests[gi].neps;ei++){
            double eps=atof(tests[gi].epslist[ei]);
        ...
        sprintf(outpath,"%s/l_s/eigenvalue-3/result/timing.txt",home);
        fp=fopen(outpath,"a");
        if(fp){fprintf(fp,"%s,%.1f,%.1f,%.2f\n",tests[gi].group,ta_ns,tc_ns,tc_ns/ta_ns);fclose(fp);}
